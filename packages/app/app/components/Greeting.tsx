...
const Greeting = () => {
  ...
  const renderGreeting = () => {
    if (getGreetingLoading) {
      return <p className="text-lg text-center text-gray-500 italic">Loading...</p>;
    }
    
    return (
      <p className={`text-lg text-center ${getGreetingError ? 'text-red-500' : ''}`}>
        {getGreetingError ? "There was an error getting the greeting" : greeting}
      </p>
    );
  };

  const renderButtonContent = () => {
    if (setGreetingLoading) {
      return "Setting greeting...";
    }

    return "Set your new greeting on the blockchain";
  };
      ...
      <div className="space-y-8">
        <div className="flex flex-col space-y-4">
          ...
          {!address ? (
            ...
            !newGreeting && (
              <p className="text-sm text-gray-500 text-center">Type something to set a new greeting</p>
            )
          )}

          {setGreetingError && (
            <p className="text-sm text-red-500 text-center">There was an error setting your new greeting</p>
          )}

          {newGreeting && prepareSetGreetingError && (
            <p className="text-sm text-red-500 text-center">Sorry, only the contract owner can set a greeting</p>
          )}
        </div>
      </div>
    </div>
